...
<div class="glass-panel p-5 rounded-[2rem] space-y-5 relative z-30 group/customer transition-all duration-300 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] border border-ink/10 bg-surface/30 backdrop-blur-md">
  <div class="absolute inset-0 bg-gradient-to-br from-ink/5 via-transparent to-transparent opacity-40 rounded-[2rem] pointer-events-none"></div>

  <div class="relative z-10 flex items-center justify-between">
    <div class="flex items-center gap-3">
       <div class="h-6 w-1 rounded-full bg-accent"></div>
  ...
  {#if activeCustomer}
    <div class="relative z-10 flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-accent/12 to-accent-hover/10 border border-accent/25 shadow-inner group-hover/customer:border-accent/35 transition-colors">
      <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-accent to-accent-hover flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-accent/20">
      ...
      {#if activeCustomer.loyalty_points}
         <div class="flex flex-col items-end">
            ...
            <span class="text-accent font-bold font-mono">{activeCustomer.loyalty_points}</span>
         </div>
      {/if}
    </div>
  {:else if addCustomerMode}
    <div class="relative z-10 space-y-3 animation-fade-in bg-surface/20 p-4 rounded-2xl border border-ink/10">
      <h4 class="text-sm font-bold text-ink">New Customer</h4>
      <div class="space-y-2">
        <input 
          class="w-full bg-bg/50 border border-ink/15 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-accent/40 focus:border-accent/40 focus:outline-none transition-all placeholder-muted/50"
          placeholder="Name *" 
      ...
      <div class="flex gap-2 justify-end pt-1">
        ...
        <button 
          class="px-4 py-2 text-xs bg-accent hover:bg-accent-hover text-white rounded-xl shadow-lg shadow-accent/20 transition-all font-bold tracking-wide active:scale-[0.98]"
  ...
  {:else}
    <div class="relative z-10">
      <div class="flex gap-2 relative z-50">
        <div class="relative flex-1 group/search">
           <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg class="h-4 w-4 text-muted group-focus-within/search:text-accent transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
           </div>
          <input 
            class="w-full bg-bg/50 border border-ink/15 rounded-xl pl-9 pr-3 py-3 text-sm shadow-inner focus:ring-2 focus:ring-accent/40 focus:border-accent/40 focus:outline-none transition-all placeholder-muted/50"
        ...
        <button 
          class="p-3 bg-accent/10 hover:bg-accent/20 text-accent rounded-xl border border-accent/25 transition-all active:scale-[0.95]"
            ...
            <button 
              ...
              class={`w-full text-left p-3.5 transition-all outline-none border-l-2 flex items-center justify-between group/opt ${
                i === activeIndex
                  ...
                  : "hover:bg-surface/40 border-transparent hover:border-ink/15"
              }`}
            ...
            >
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2">
                  <p class={`font-bold text-sm truncate transition-colors ${i === activeIndex ? "text-accent" : "text-ink group-hover/opt:text-accent"}`}>{customer.name}</p>
                </div>
              ...
              {#if customer.loyalty_points != null}
                <span class="shrink-0 text-[10px] font-bold text-accent bg-accent/10 border border-accent/20 rounded-md px-1.5 py-0.5">
      ...
      <div class="mt-4 text-center">
        <button 
          class="text-xs font-bold uppercase tracking-wide text-accent hover:text-accent-hover transition-colors flex items-center justify-center gap-1 mx-auto group/create"
          on:click={() => addCustomerMode = true}
        >
          <span class="bg-accent/10 rounded-full p-1 group-hover/create:bg-accent/20 transition-colors">
