...
struct OnboardParams {
  ...
  // Optional: where to store local-node runtime (env, compose, onboarding output) when using bundled mode.
  edge_home: Option<String>,

  api_port: Option<u16>,
  admin_port: Option<u16>,

  // For "pos" mode, this is the remote API base URL the provisioning calls should use.
...
fn ensure_edge_bundle(app: &tauri::AppHandle, edge_home: &Path) -> Result<PathBuf, String> {
  ...
  if !src_runner.exists() || !src_compose.exists() {
    return Err("Bundled local-node assets are missing from this installer build.".to_string());
  }
...
fn check_prereqs(app: tauri::AppHandle, repo_path: String) -> Result<Prereqs, String> {
  ...
  if !repo_ok {
    details.push(
      "Missing onboarding runner. Either provide a valid repo path, or reinstall Setup Desktop with bundled local-node assets."
        .to_string(),
    );
  }
  if !docker_ok {
...
fn write_env_file(path: &Path, values: &std::collections::HashMap<String, String>) -> Result<(), String> {
  ...
  lines.push("# Local node service ports".to_string());
  ...
  lines.push("".to_string());
  lines.push("# Node -> cloud sync (optional)".to_string());
  lines.push(format!(
...
fn wait_api_healthy(app: &tauri::AppHandle, state: &Arc<Mutex<RunnerState>>, api_base: &str, timeout_s: u64) -> Result<(), String> {
  ...
  Err(format!("Store API did not become healthy in time ({timeout_s}s). Last error: {last_err}"))
}
...
fn run_onboarding_internal(app: &tauri::AppHandle, state: &Arc<Mutex<RunnerState>>, params: OnboardParams) -> Result<(), String> {
  ...
  if compose_mode_images {
    // Ensure the app's bundled local-node assets exist under edge_home (overwrites on each run).
  ...
  if !skip_start {
    emit_log(app, "Starting local node stack...");
    ...
    emit_log(app, "Skipping local node start (POS-only mode).");
  }
  ...
  emit_log(app, &format!("Waiting for API health at {api_base}/health ..."));
  wait_api_healthy(app, state, &api_base, 300)?;
  emit_log(app, "API is healthy.");
  ...
  if !devices.is_empty() {
    ...
    let prefill = json!({
      ...
      "cloudUrl": cloud_api_url.clone(),
      "edgeLanUrl": edge_api_url_for_pos.clone(),
      "edgeUrl": if cloud_api_url.is_empty() { edge_api_url_for_pos.clone() } else { cloud_api_url.clone() },
      "portOfficial": 7070,
      "portUnofficial": 7072,
  ...
  emit_log(app, &format!("- POS API URL: {}", edge_api_url_for_pos));
  if sync_enabled {
    emit_log(app, &format!("- Node->Cloud sync target: {}", cloud_api_url));
  } else {
    emit_log(app, "- Node->Cloud sync: disabled");
  }
