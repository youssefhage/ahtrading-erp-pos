#!/usr/bin/env python3
"""
Guided onboarding runner for on-prem local-node + POS device provisioning.

What it does:
1) Generates deploy/edge/.env.edge from safe defaults + your inputs.
2) Starts the local node stack (docker compose).
...
def _wait_api_healthy(base_url: str, timeout_s: int = 300) -> None:
    ...
    while (time.time() - start) < timeout_s:
        try:
            ...
            last_err = str(ex)
        time.sleep(2)
    raise RuntimeError(f"Store API did not become healthy in time ({timeout_s}s). Last error: {last_err}")
...
def _edge_compose_cmd(compose_path: Path, env_path: Path, *extra: str) -> list[str]:
    return [
        "docker",
        "compose",
        "--env-file",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Automate on-prem local-node + POS onboarding without touching cloud Dokploy.",
    )
    parser.add_argument(
        "--edge-home",
        default="",
        help="Folder to store local-node runtime state (env, onboarding output). "
        "Default: deploy/edge (repo mode) or current folder (bundle mode).",
    )
    parser.add_argument(
        "--compose-mode",
        choices=["build", "images"],
        default="build",
        help="How to start the local-node stack. "
        "'build' builds images locally (dev). "
        "'images' pulls prebuilt images (recommended for end-user installs).",
    )
    parser.add_argument("--non-interactive", action="store_true", help="Do not prompt for input.")
    parser.add_argument("--force", action="store_true", help="Overwrite .env.edge without confirmation.")
    parser.add_argument(
        "--update-env",
        action="store_true",
        help="Update .env.edge in-place (ports/sync/bootstrap). Default is to reuse existing env unchanged.",
    )
    parser.add_argument("--skip-start", action="store_true", help="Do not start or restart local node compose stack.")
    parser.add_argument("--skip-devices", action="store_true", help="Skip POS device registration and pack export.")
    parser.add_argument(
        "--api-base-url",
        default="",
        help="Override local API base URL for onboarding calls (health/login/register). "
        "Useful when registering devices against a remote server. Example: http://192.168.1.50:8001",
    )
    parser.add_argument("--api-port", type=int, default=None, help="local API port on the on-prem server.")
    parser.add_argument("--admin-port", type=int, default=None, help="local admin UI port on the on-prem server.")
    parser.add_argument(
        "--edge-api-url-for-pos",
        default="",
        help="URL POS terminals should use (example: http://192.168.1.50:8001).",
    )
    parser.add_argument("--cloud-api-url", default=DEFAULT_CLOUD_API_URL, help="Cloud API URL for node->cloud sync.")
    parser.add_argument("--edge-sync-key", default="", help="Shared secret for node->cloud sync.")
    parser.add_argument("--edge-node-id", default="", help="Unique sync node id (store identifier).")
    parser.add_argument("--admin-email", default=None, help="Bootstrap admin email for local server.")
    parser.add_argument("--admin-password", default=None, help="Bootstrap admin password for local server.")
    ...
    edge_home_raw = (args.edge_home or "").strip()
    if edge_home_raw:
        edge_home = Path(edge_home_raw).expanduser().resolve()
    else:
        # Repo mode default: deploy/edge. Bundle mode default: current folder.
        edge_home = (repo_root / "deploy" / "edge").resolve()
        if not edge_home.exists():
            edge_home = Path.cwd().resolve()

    env_path = edge_home / ".env.edge"
    onboarding_root = edge_home / "onboarding"

    compose_mode = str(args.compose_mode or "build").strip().lower()
    if compose_mode == "images":
        compose_file = (edge_home / "docker-compose.edge.images.yml").resolve()
        if not compose_file.exists():
            compose_file = (repo_root / "deploy" / "edge" / "docker-compose.edge.images.yml").resolve()
    else:
        compose_file = (repo_root / "deploy" / "docker-compose.edge.yml").resolve()

    print("== On-Prem + POS Onboarding ==")
    print("This flow only manages local on-prem node and POS setup.")
    print(f"Local node home: {edge_home}")
    ...
    if not edge_api_url_for_pos:
        ...
        edge_api_url_for_pos = _ask(
            "POS API URL (LAN address that cashiers will use)",
            default=default_url,
    ...
    if not sync_enabled and not non_interactive:
        enable_sync = _ask("Enable node->cloud sync now", default="y")
    ...
    if not args.skip_start:
        ...
        print("Starting local node stack...")
        up_args = ["up", "-d"]
        cwd = repo_root
        if compose_mode == "images":
            # For image-based installs, pull updates automatically.
            up_args.extend(["--pull", "always"])
            cwd = edge_home
        else:
            up_args.append("--build")
        _run(_edge_compose_cmd(compose_file, env_path, *up_args), cwd=cwd)
    else:
        print("Skipping local node start (--skip-start).")
    ...
    if should_write_env:
        ...
        if not args.skip_start:
            ...
            refresh_args = ["up", "-d"]
            refresh_cwd = repo_root
            if compose_mode == "images":
                refresh_args.extend(["--pull", "always"])
                refresh_cwd = edge_home
            _run(_edge_compose_cmd(compose_file, env_path, *refresh_args), cwd=refresh_cwd)

    print("")
    print("Onboarding complete.")
    print(f"- POS API URL: {edge_api_url_for_pos}")
    if sync_enabled:
        print(f"- Node->Cloud sync target: {cloud_api_url}")
    else:
        print("- Node->Cloud sync: disabled")
