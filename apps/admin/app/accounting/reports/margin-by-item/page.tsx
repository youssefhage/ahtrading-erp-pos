...
export default function MarginByItemPage() {
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => [
    { id: "sku", accessorFn: (r) => r.sku || "-", header: ({ column }) => <DataTableColumnHeader column={column} title="SKU" />, cell: ({ row }) => <span className="font-mono text-sm">{row.original.sku || "-"}</span> },
    { id: "name", accessorFn: (r) => r.name || "-", header: ({ column }) => <DataTableColumnHeader column={column} title="Item" /> },
    { id: "qty_sold", accessorFn: (r) => toNum(r.qty_sold), header: ({ column }) => <DataTableColumnHeader column={column} title="Qty" />, cell: ({ row }) => <div className="text-right font-mono text-sm">{toNum(row.original.qty_sold).toLocaleString("en-US")}</div> },
    {
