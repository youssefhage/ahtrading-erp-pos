...
export default function JournalTemplateEditPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMemo(detail.template.memo || "");
      setIsActive(Boolean(detail.template.is_active));
      setDefaultRateType(String(detail.template.default_rate_type || "market"));
      setLines(
        (detail.lines || []).map((l) => ({
          key: l.id,
          side: l.side,
  ...
  async function save(e: React.FormEvent) {
    ...
    setStatus("Saving...");
    try {
      await apiPatch(`/accounting/journal-templates/${encodeURIComponent(id)}`, {
        name: name.trim(),
        memo: memo.trim() || null,
        is_active: isActive,
      ...
      router.push(`/accounting/journal-templates/${encodeURIComponent(id)}`);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setStatus(message);
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Edit Journal Template</CardTitle>
            ...
            <div className="rounded-md border border-border bg-bg-elevated p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-fg-muted">Template must balance.</div>
                </div>
                ...
                {lines.map((l, idx) => (
                  <div key={l.key} className="grid grid-cols-1 gap-2 rounded-md border border-border-subtle bg-bg-sunken p-2 md:grid-cols-[110px_160px_1fr_160px_160px_150px_150px_40px]">
                    <div className="space-y-1">
                      ...
                      <select className="ui-select w-full" value={l.side} onChange={(e) => updateLine(idx, { side: e.target.value as any })}>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input value={l.account_code} onChange={(e) => onAccountCodeChange(idx, e.target.value)} />
                      {!l.account_id && l.account_code ? <div className="text-[11px] text-danger">Unknown code</div> : null}
                    </div>

                    <div className="space-y-1">
                      <label className="text-[11px] font-medium text-fg-muted">Memo</label>
                      <Input value={l.memo} onChange={(e) => updateLine(idx, { memo: e.target.value })} />
                    </div>
