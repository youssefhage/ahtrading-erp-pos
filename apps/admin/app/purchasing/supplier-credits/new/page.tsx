...
export default function SupplierCreditNewPage() {
  ...
  const [lines, setLines] = useState<LineDraft[]>([
    { key: "l1", description: "Supplier credit", amount_usd: "", amount_lbp: "" }
  ]);
  ...
  const loadReceipts = useCallback(async () => {
    ...
    try {
      ...
      params.set("supplier_id", supplier.id);
  ...
  function addLine() {
    setLines((prev) => [
      ...prev,
      { key: `l-${Date.now()}-${prev.length + 1}`, description: "", amount_usd: "", amount_lbp: "" }
    ]);
  }
  ...
  async function create() {
    if (!supplier?.id) return setStatus("Select a supplier first.");
    ...
    setStatus("Creating...");
    try {
      const res = await apiPost<{ id: string }>("/purchases/credits/drafts", {
      ...
      router.push(`/purchasing/supplier-credits/${encodeURIComponent(res.id)}`);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      {status ? <ErrorBanner error={status} /> : null}

      <Card>
        <CardHeader>
          <CardTitle>New Supplier Credit</CardTitle>
          <CardDescription>Create a draft credit note/rebate document.</CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div className="space-y-1">
              ...
              <SupplierTypeahead
                ...
                placeholder={supplier ? `${supplier.code ? `${supplier.code} · ` : ""}${supplier.name}` : "Search supplier..."}
              />
            </div>
            <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Dialog open={pickOpen} onOpenChange={setPickOpen}>
                    <DialogTrigger asChild>
                      <Button type="button" variant="outline" disabled={!supplier?.id}>
                        Pick
                      </Button>
                    </DialogTrigger>
