        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-2 px-4 py-3">
          <div className="flex items-center gap-2">
            <Button asChild variant="outline">
              <Link href={`/purchasing/supplier-invoices/${encodeURIComponent(id)}`}>Back</Link>
            </Button>
          </div>
          <div className="flex items-center gap-2">
        ...
        {!detail ? (
          ...
          <div className="space-y-6">
            <header className="flex flex-wrap items-start justify-between gap-4 border-b border-black/15 pb-4">
              <div>
                <h1 className="text-xl font-semibold tracking-tight">Supplier Invoice</h1>
                <p className="mt-1 font-mono text-xs text-black/70">
                  {detail.invoice.invoice_no} · {detail.invoice.status}
                  {detail.invoice.is_on_hold ? " · HOLD" : ""}
                </p>
              </div>
            ...
            <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-md border border-black/15 p-3">
                ...
                <p className="mt-1 text-sm font-medium">{detail.invoice.supplier_name || detail.invoice.supplier_id || "-"}</p>
                {detail.invoice.supplier_ref ? (
                  <p className="mt-1 font-mono text-[11px] text-black/70">Ref: {detail.invoice.supplier_ref}</p>
                ) : null}
              </div>
              <div className="rounded-md border border-black/15 p-3">
                <p className="text-[11px] uppercase tracking-wider text-black/60">Goods Receipt</p>
                <p className="mt-1 text-sm font-medium">{detail.invoice.goods_receipt_no || detail.invoice.goods_receipt_id || "-"}</p>
              </div>
              <div className="rounded-md border border-black/15 p-3">
                <p className="text-[11px] uppercase tracking-wider text-black/60">Tax Code</p>
                <p className="mt-1 text-sm font-medium font-mono">{detail.invoice.tax_code_id || "-"}</p>
                <p className="text-[11px] text-black/70">Exchange rate: <span className="font-mono">{String(detail.invoice.exchange_rate || "-")}</span></p>
              </div>
            </section>
                        ...
                        <td className="px-4 py-2">
                          ...
                          {l.batch_no ? (
                            <div className="mt-1 font-mono text-[11px] text-black/60">
                              Batch {l.batch_no}
                              {l.expiry_date ? ` · Exp ${fmtIso(l.expiry_date)}` : ""}
                            </div>
                          ) : null}
                        </td>
                        <td className="px-3 py-2">
                          <div className="font-mono text-[11px] text-black/70">{l.supplier_item_code || "-"}</div>
