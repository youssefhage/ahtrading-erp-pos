...
function SalesPaymentsInner() {
  ...
  const columns = useMemo<ColumnDef<SalesPaymentRow>[]>(() => [
    ...
    { id: "invoice", accessorFn: (p) => p.invoice_no, header: ({ column }) => <DataTableColumnHeader column={column} title="Invoice" />, cell: ({ row }) => <span className="font-mono text-sm">{row.original.invoice_no}</span> },
