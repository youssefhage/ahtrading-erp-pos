...
function SalesPaymentsInner() {
  ...
  const columns = useMemo<ColumnDef<SalesPaymentRow>[]>(() => [
    ...
    { id: "invoice", accessorFn: (p) => p.invoice_no, header: ({ column }) => <DataTableColumnHeader column={column} title="Invoice" />, cell: ({ row }) => <Link href={`/sales/invoices/${row.original.invoice_id}`} className="font-mono text-sm text-blue-600 underline-offset-4 hover:underline dark:text-blue-400">{row.original.invoice_no}</Link> },
