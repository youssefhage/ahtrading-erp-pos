...
def _auto_balance_journal(cur, company_id: str, journal_id: str, *, warehouse_id=None):
    """Post a tiny rounding line if the journal is slightly off due to LBP round-trip noise."""
    cur.execute(
        """
        SELECT
    ...
    defaults = fetch_account_defaults(cur, company_id)
    rounding_acc = defaults.get("ROUNDING")
    if not rounding_acc:
        raise ValueError("journal is imbalanced; missing ROUNDING account default")
    debit_usd = credit_usd = debit_lbp = credit_lbp = Decimal("0")
    if diff_usd > 0:
        credit_usd = diff_usd
    else:
    ...
    cur.execute(
        ...
        VALUES (gen_random_uuid(), %s, %s, %s, %s, %s, %s, 'Rounding (auto-balance)', %s)
        """,
        (journal_id, rounding_acc, debit_usd, credit_usd, debit_lbp, credit_lbp, warehouse_id),
    )
...
def process_sale(cur, company_id: str, event_id: str, payload: dict, device_id: str):
    ...
    _auto_balance_journal(cur, company_id, journal_id, warehouse_id=warehouse_id)
