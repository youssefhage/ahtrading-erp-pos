...
# Run all numeric migrations in sorted order.
# This prevents drift where application code starts relying on a migration but init_db.sh
# wasn't manually updated to include it.
for file in backend/db/migrations/[0-9][0-9][0-9]_*.sql; do
